    ...
    for stock in context.portfolio.positions:
        if not get_open_orders(stock):
            ...
            if np.isnan(SellPrice):
                ...
                and context.MyRetreatAge<=context.age[stock] 
                and (
                    context.MyRetreatPrice>CurrPrice
                    or CostBasis>CurrPrice
                )
            ):
                SellPrice = float(
                    CostBasis-(
                        context.age[stock]/(100.00*context.MyDaysPerPennyReduction)
                    )
                )
